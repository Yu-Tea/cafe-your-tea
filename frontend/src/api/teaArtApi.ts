...
// TeaArt作成
export const createTeaArt = async (data: CreateTeaArtRequest): Promise<TeaArtResponse> => {
...
// TeaArt更新
export const updateTeaArt = async (id: number, data: CreateTeaArtRequest): Promise<TeaArtResponse> => {
  const response = await apiClient.patch(`/tea_arts/${id}`, data);
  return response.data;
};

// TeaArt削除
export const deleteTeaArt = async (id: number): Promise<{ message: string }> => {
